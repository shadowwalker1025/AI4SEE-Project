import React from "react";
import "./features.css";
import Nav from "./Nav";
import c1 from "../../assets/counter_icon1.png";
import c4 from "../../assets/counter_icon4.png";
import c2 from "../../assets/counter_icon2.png";
import c3 from "../../assets/counter_icon3.png";
import monitor from "../../assets/laptop1.png";
import certficate from "../../assets/certificate.png";
import Footer from "../../components/LiveStates/Footer";
function Features() {
  return (
    <div>
      <Nav />
      <div className="backl">
        <div className="container  p-5">
          <h1 className="text-center text-light">Live lectures with coding</h1>
          <p className="text-center text-light">
            Learn from our tutors live and interact for doubt clearance.{" "}
          </p>
          <div>
            <div className="row p-5">
              <div className="col-sm-4">
                <div class="card bg-transparent border-1">
                  <img class="card-img-top tut" src={c1} alt="Card image cap" />
                  <div class="card-body">
                    <h5 class="card-title text-light">Job preparation</h5>
                    <p class="card-text text-light">
                      Become job ready with our interview guidance and machine
                      learning job interview case studies.
                    </p>
                    <a href="#" class="btn btn-primary">
                      Go somewhere
                    </a>
                  </div>
                </div>
              </div>
              <div className="col-sm-4">
                <div class="card bg-transparent border-1">
                  <img class="card-img-top tut" src={c4} alt="Card image cap" />
                  <div class="card-body">
                    <h5 class="card-title text-light">Expert Mentorship</h5>
                    <p class="card-text text-light">
                      Interact with expert tutors for doubt clearance and
                      guidance
                    </p>
                    <a href="#" class="btn btn-primary">
                      Go somewhere
                    </a>
                  </div>
                </div>
              </div>
              <div className="col-sm-4">
                <div class="card bg-transparent border-1">
                  <img
                    class="card-img-top tut"
                    src={monitor}
                    alt="Card image cap"
                  />
                  <div class="card-body">
                    <h5 class="card-title text-light">
                      Developer and discussion forum
                    </h5>
                    <p class="card-text text-light">
                      Life time access to our community forums for anytime doubt
                      clearance.
                    </p>
                    <a href="#" class="btn btn-primary">
                      Go somewhere
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <div className="row p-5">
              <div className="col-sm-4">
                <div class="card bg-transparent border-2">
                  <img class="card-img-top tut" src={c2} alt="Card image cap" />
                  <div class="card-body">
                    <h5 class="card-title text-light">
                      Certificate on completion
                    </h5>
                    <p class="card-text text-light">
                      We offer certificate programs in machine learning..
                    </p>
                    <a href="#" class="btn btn-primary">
                      Go somewhere
                    </a>
                  </div>
                </div>
              </div>
              <div className="col-sm-4">
                <div class="card bg-transparent border-2">
                  <img class="card-img-top tut" src={c3} alt="Card image cap" />
                  <div class="card-body">
                    <h5 class="card-title text-light">
                      Internship opportunity
                    </h5>
                    <p class="card-text text-light">
                      Some quick example text to build on the card title and
                      make up the bulk of the card's content.
                    </p>
                    <a href="#" class="btn btn-primary">
                      clikl
                    </a>
                  </div>
                </div>
              </div>
              <div className="col-sm-4">
                <div class="card bg-transparent border-2">
                  <img
                    class="card-img-top tut"
                    src={certficate}
                    alt="Card image cap"
                  />
                  <div class="card-body">
                    <h5 class="card-title text-light">Card title</h5>
                    <p class="card-text text-light">
                      Some quick example text to build on the card title and
                      make up the bulk of the card's content.
                    </p>
                    <a href="#" class="btn btn-primary">
                      Go somewhere
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <Footer />
    </div>
  );
}

export default Features;
